...
amebit_monthly %>%
  ...
  geom_line(alpha = 0.5) +
  facet_wrap(~ state_abb, ncol = 2) +  # 2 columns, adjust as needed
  labs(
...
Seasonal shifts in average temperature, precipitation, and number of American Bittern has all increased from 2000 to 2024 (Fig. 9). 

```{r fig.cap="Figure 8: Seasonality shifts of average temperature, precipitation, and number of American Bittern observations recorded yearly from 2000-2024. Data was recorded within nine weather stations across Arizona, California, Oregon, and Washington, including eBird observation count recorded between 2000 and 2024 for the respective states.", fig.align='center'}
...
ggplot(combined_long, aes(x = value, y = observations, color = season)) +
  geom_point(alpha = 0.7) +
  facet_wrap(state_abb ~ variable, scales = "free_x", 
             labeller = labeller(variable = c(
               avg_temp = "Average Temperature (°F)",
               avg_precp = "Average Precipitation (mm)"
             ))) +
  labs(
    title = "Seasonal Temperature, Precipitation, and American Bittern Observations",
    x = NULL,
    y = "Number of Observations",
    color = "Season"
  ) +
...
```{r fig.cap="Figure 9: Average temperature of year (2000-2024) in relation towards American Bittern observation count faceted by seasonality. Each point represents a year ranging from 2000-2024, with it's average temperature measured, by season, and number of recorded American Bittern observations. Recorded within Arizona, California, Oregon, and Washington.", fig.align='center'}
...
```{r fig.cap="Figure 11: Average temperature (mm) of year (2000-2024) effect on American Bittern observation count taken during that time period, within Arizona, California, Oregon, and Washington. Points represent individual observations, and the blue line shows a linear trend with 95% confidence interval.", fig.align='center'}
...
```{r fig.cap="Figure 12: Interannual variation in American Bittern observations from 2000 to 2024. Spread of observations across years, highlighting changes over time.", fig.align='center'}
...
```{r fig.cap="Figure 13: Predicted effect of average temperature on American Bittern observations from the mixed effects model. Marginal predictions are based on fixed effects, holding precipitation, year, and state constant.", fig.align='center'}
...
```{r fig.cap="Figure 14 & 15: Random intercept effects of season_year and state on American Bittern observations from the mixed model. Positive values indicate above-average observation rates relative to the grand mean.", fig.align='center'}
library(sjPlot)
